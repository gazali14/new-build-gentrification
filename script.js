...
function initMap() {
  ...
  function getKec(layer) {
    return lookup[(layer.feature?.properties.nama_kec||layer.feature?.properties.KECAMATAN||"").toUpperCase().trim()];
  }
  ...
  function getLayerEl(layer) {
    return (typeof layer.getElement === "function" ? layer.getElement() : null) || layer._path || null;
  }
  ...
  function filteredStyle(d) {
    if (top5Active) {
      ...
      return { opacity:isTop?1:0.04, fillOpacity:isTop?0.95:0.02, weight:isTop?3:0.4 };
    }
    if (activeRisk !== "all") {
  ...
  function applyCategoryFilter(cat) {
    activeRisk = cat;
    document.querySelectorAll(".peta-btn[data-risk]").forEach(b => b.classList.toggle("active", b.dataset.risk === cat));
  ...
  document.querySelectorAll(".legend-row[data-filter]").forEach(row => {
    row.addEventListener("click", () => applyCategoryFilter(row.dataset.filter));
  });
  ...
  document.getElementById("btn-top5")?.addEventListener("click", function() {
    ...
    if (top5Active) {
      window._kecLayer?.eachLayer(layer => {
        const d = getKec(layer);
        const isTop = TOP5.includes(d?.kecamatan);
        layer.setStyle({ opacity:isTop?1:0.04, fillOpacity:isTop?0.95:0.02, weight:isTop?3:0.4 });
        ...
        plugins:{
          legend:{
            ...
            labels:{
              ...
              generateLabels: () => [
                { text:"Probabilitas prediksi (warna = kategori)", fillStyle:"#ff6820", strokeStyle:"#ff6820", fontColor:"rgba(200,192,176,0.85)", pointStyle:"circle", datasetIndex:0 },
                { text:"Median bootstrap (50×)", fillStyle:"transparent", strokeStyle:"rgba(240,234,216,0.75)", fontColor:"rgba(200,192,176,0.85)", lineWidth:1.5, pointStyle:"rectRot", datasetIndex:1 },
              ],
            },
            onClick: () => {},
          },
