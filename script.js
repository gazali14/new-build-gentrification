...
function initMap() {
  ...
  function onHover(e) {
    ...
    const d =
      lookup[
        (
          e.target.feature.properties.nama_kec ||
          e.target.feature.properties.KECAMATAN ||
          ""
        )
          .toUpperCase()
          .trim()
  ...
  document.querySelectorAll(".legend-row[data-filter]").forEach((row) => {
    row.addEventListener("click", () => {
      ...
      document
        ...
        .forEach((b) => b.classList.remove("active"));
      document
        .querySelector(`.peta-btn[data-risk="${cat}"]`)
  ...
  document.getElementById("btn-reset")?.addEventListener("click", () => {
    ...
    window._kecLayer?.eachLayer((l) => geojsonLayer.resetStyle(l));
    document
      .querySelectorAll(".peta-btn[data-risk]")
      .forEach((b) => b.classList.toggle("active", b.dataset.risk === "all"));
  });
